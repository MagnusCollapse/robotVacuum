...
import vacuumCleanerRobot.MOMGame;
import vacuumCleanerRobot.MyMap;
...
public class TestObject implements GameObject, LocatedRectangle {
	...
	public TestObject(Dimension size, Point position, MyMap map) throws IOException {
		this.size = size;
	...
	public void update(long deltaTime) {
		if ((this.position.x + this.size.width > (int) MOMGame.getWindowBounds().getWidth()) || this.position.x < 0) {
			// this.position.x=-this.size.width;
			speed2D.setVectorX(-speed2D.VectorX());

		}
		if ((this.position.y + this.size.height > (int) MOMGame.getWindowBounds().getHeight()) || this.position.y < 0) {
			// this.position.x=-this.size.width;
